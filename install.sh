...
if [[ $DO_DEPS -eq 0 && $DO_UNINSTALL -eq 0 && $DO_BIN_ONLY -eq 0 && -t 0 ]]; then
    read -r -p "[crun install] just grab the portable binary, nothing else? [y/N] " reply
    if [[ "$reply" =~ ^[Yy] ]]; then
        read -r -p "[crun install] where to? [default: $HOME] " path_reply
        do_bin_only "${path_reply:-$HOME}"
        exit 0
    fi

    read -r -p "[crun install] also install per-language toolchain dependencies via crun --deps? [y/N] " reply
    if [[ "$reply" =~ ^[Yy] ]]; then
        DO_DEPS=1
        read -r -p "[crun install] only one language (leave empty for all)? " DEPS_TARGET
    fi

    read -r -p "[crun install] install under a custom binary name instead of 'crun'? (leave empty to skip) " name_reply
    if [[ -n "$name_reply" ]]; then
        INSTALL_NAME="$name_reply"
        INSTALL_TARGET="$BIN_DIR/$INSTALL_NAME"
    fi

    read -r -p "[crun install] copy the binary instead of symlinking it? [y/N] " reply
    [[ "$reply" =~ ^[Yy] ]] && DO_COPY=1
fi
