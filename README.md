...
Or install directly without cloning first — the installer detects a piped run and clones the repo to `~/Projects/CRun` for you:
...
Requires Rust (`cargo`). Builds a release binary and symlinks it to `~/.local/bin/crun`.
...
```bash
./install.sh          # rebuilds and the existing symlink picks it up automatically
```
