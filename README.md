...
Want to invoke it as something other than `crun` (e.g. `runfile`)? Pass `--name`/`-Name` at install time — it controls what the installed symlink/copy is named, the actual `crun` build is untouched:

```bash
./install.sh --name runfile              # -> ~/.local/bin/runfile
./install.sh --name runfile --copy       # combine with other flags freely
...
Don't want a symlink, PATH changes, or anything installed — just the portable
binary copied somewhere? `--bin`/`-Bin` grabs the tracked [`bin/crun`](bin/) (or
builds it if missing) and copies it to a path of your choice, defaulting to `$HOME`:

```bash
./install.sh --bin              # -> ~/crun
./install.sh --bin /opt/tools   # -> /opt/tools/crun
```
```powershell
.\install.ps1 -Bin                # -> ~\crun.exe
.\install.ps1 -Bin C:\Tools       # -> C:\Tools\crun.exe
```

Running the installer with no flags at all now asks interactively what you
want — bin-only is the first thing it offers.
