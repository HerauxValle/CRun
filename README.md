...
Want to invoke it as something other than `crun` (e.g. `runfile`)? Pass `--bin`/`-Bin` at install time — it controls what the installed symlink/copy is named, the actual `crun` build is untouched:

```bash
./install.sh --bin runfile              # -> ~/.local/bin/runfile
./install.sh --bin runfile --copy       # combine with other flags freely
./install.sh --uninstall --bin runfile  # uninstall respects the custom name too
```
```powershell
.\install.ps1 -Bin runfile              # -> ~\.local\bin\runfile.exe
.\install.ps1 -Bin runfile -Uninstall   # uninstall respects the custom name too
```
...
C, C++, and Objective-C get `-Wall -Werror` by default. Go and C# rely on their own compiler strictness (no warning flags injected). Rust gets `-D warnings` and Swift gets `-warnings-as-errors` as their equivalents. Pass `--no-werror` to drop the "promote warnings to errors" behavior across the board (C/C++/ObjC keep `-Wall`, Rust/Swift skip their strict flags).

**Adding a language**: just drop `src/languages/mylang.rs` implementing `pub fn config() -> CompilerConfig`. `build.rs` scans `src/languages/` at build time and auto-generates the module declarations and registry — nothing else to wire up, nothing else to touch.
...
OPTIONS:
    ...
    -t, --test-compile [<LANG>] Compile and run the bundled test(s); no value or "all" runs every
                                language in order. Valid: c, cpp, cs, go, rs, swift, m
    -h, --help                  Print help
