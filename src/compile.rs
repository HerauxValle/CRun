    ...
    match build_system {
        BuildSystem::Make => {
            ...
            if guessed.exists() {
                Ok(CompileOutput {
                    binary_path: guessed,
                    execution_mode: crate::languages::ExecutionMode::Native,
                })
            } else {
...
fn compile_sources(
    files: &[PathBuf],
    entry: Option<&Path>,
    config: &crate::languages::CompilerConfig,
    out_path: &Path,
    no_werror: bool,
) -> Result<CompileOutput, String> {
    ...
    if !is_go && !is_csharp && !is_rust && !is_swift {
        cmd.arg("-Wall");
        if !no_werror {
            cmd.arg("-Werror");
        }
    ...
    // Output flag — all languages that reach here use -o except C#
    if !is_csharp {
    ...
    for entry in entries.flatten() {
        ...
        if path.is_file() {
            if let Ok(meta) = path.metadata() {
                // Check if the file has any execute bit set
