...
use crate::languages::{self, CompilerConfig};
...
fn detect_file(path: &Path) -> Result<DetectResult, String> {
    let config = languages::find_config(path).ok_or_else(|| {
        let ext = path
            .extension()
            .and_then(|e| e.to_str())
...
fn detect_dir(dir: &Path) -> Result<DetectResult, String> {
    ...
    for file in &source_files {
        if let Some(cfg) = languages::find_config(file) {
            by_language
                .entry(cfg.name)
...
fn collect_source_files(dir: &Path) -> Result<Vec<PathBuf>, String> {
    let known_exts: HashSet<&'static str> = languages::all_languages()
        .iter()
